...
----------------------------------------------------------------------------?
Billboard Join Repair + Time-Series Snapshot Builder
----------------------------------------------------------------------------?

PURPOSE:
  Fixes the broken years_on_billboard join and builds a properly structured
...
LEAKAGE DISCLOSURE (documented per project decision):
...
# Legacy artist thresholds (time-gated: computed from ALL history before cutoff_year)
# An artist is a "legacy act" if they have deep historical chart presence
# even if their recency score has decayed. Captures Dr. Dre, Eminem, Mary J. Blige
# type artists who Roc Nation selects for cultural weight, not current chart activity.
LEGACY_MIN_TOP25_SONGS  = 3   # at least 3 top-25 song appearances all-time
LEGACY_MIN_CHART_YEARS  = 3   # at least 3 years on either chart all-time
LEGACY_MIN_PEAK_SCORE   = 50  # peak song score >= 50 (i.e. rank <= 51)
...
# --------------------------------------------?
#  SECTION 2 - BUILD ARTIST LABEL SETS
#  (Roc Nation SB headliners + guests, per year)
# --------------------------------------------?
...
def compute_billboard_features_as_of(df_artists, df_songs_exp, artist_name, cutoff_year):
    ...
    # Legacy flag: deep historical impact regardless of recency
    # True if artist meets ANY of the legacy thresholds.
    # Designed to capture: Dr. Dre, Eminem, Mary J. Blige, Usher, JLo, Shakira
    # who are culturally massive but haven't charted recently.
    is_legacy = int(
    ...
    # Legacy-adjusted recency: for legacy artists, floor the recency score at 0.4
    # so they are never fully penalized for not charting recently.
    # Non-legacy artists keep their raw recency score.
...
for a in section_artists:
    ...
    print(f"  {a:<25} {f['years_on_billboard']:>4} "
          ...
          f"{f['legacy_adjusted_recency']:>8.2f} "
          f"{f['is_legacy_act']:>7}")
...
agg_dict = {
    ...
    'years_on_billboard' : 'max',   # keep original (will be REPLACED by billboard join below)
}
...
# --------------------------------------------?
#  SECTION 5 - NAME MATCHING DIAGNOSTICS
...
# --------------------------------------------?
#  SECTION 6 - BUILD TIME-SERIES TRAINING TABLE
...
# --------------------------------------------?
#  SECTION 8 - CROSS-VALIDATION FOLD PREVIEW
# --------------------------------------------?
...
section("Summary")
print(f"""
  Training table built successfully.

  Key design decisions:
    [OK] Billboard features are TIME-GATED (no leakage)
      - years_on_billboard, peak_rank, recency, top10 appearances
        are all computed from data strictly before each SB year

    [WARNING] Spotify features are STATIC SNAPSHOTS (leakage documented)
      - artist_popularity, artist_followers, track_popularity,
        and audio features reflect current values, not year-Y values

    [OK] Forward-chaining CV structure defined (6 folds, 2019-2025)
    [OK] 2026 prediction candidates isolated in prediction_candidates_2026.csv

  Next steps:
    1. Run soundcharts_enrichment.py to fill audio features for top candidates
    2. Merge enriched audio features into training_table.csv
    3. Build scoring model using training_table.csv with forward-chaining CV
""")