...
What `eda_dataset.py` does: loads `final_training_dataset.csv` and only explores the data (no training or predictions). It shows (1) a pie chart of performer vs non-performer labels to highlight class imbalance, (2) a numeric correlation heatmap, (3) a boxplot of track popularity by label, and (4) a radar chart comparing average audio features (danceability, energy, valence, acousticness) for performers vs non-performers. Run: `python eda_dataset.py`.
